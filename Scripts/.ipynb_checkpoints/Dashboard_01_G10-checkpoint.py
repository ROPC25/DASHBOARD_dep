...
def plot_distribution(applicationDF, feature, client_feature_val, title):
    applicationDF.loc[applicationDF["DAYS_EMPLOYED"] > 0, "DAYS_EMPLOYED"] = np.nan # remplacer >0 par nan (ex: +365243)    
    ...
    if feature == "DAYS_BIRTH":
        ...
        sns.kdeplot((t0[feature] / 365).dropna(), label='Remboursé', color='g', ax=ax)
        sns.kdeplot((t1[feature] / 365).dropna(), label='Défaillant', color='r', ax=ax)
        if client_feature_val>0:
            client_feature_val=-36500
        ax.axvline(float(client_feature_val / 365), color="blue", linestyle='--', label='Client')
...
if show_client_comparison:
    ...
    if pd.api.types.is_numeric_dtype(data_train[feature]):
        ...
        rotate_lbl = n_unique > 2       # Rotation si trop de catégories (ex:4 cat)
    ...
    def plot_distribution_Histogram(selected_feature, col):
    ...
    plot_distribution_Histogram(selected_feature_positive, col1)
    plot_distribution_Histogram(selected_feature_negative, col2)
